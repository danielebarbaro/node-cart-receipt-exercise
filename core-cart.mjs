...
//funzione getUser dichiartiva 
function getUser(uuid) {
    let user = users.find(user =>uuid === user.uuid);
    return user; 
}

//funzione getUser anonima
/*const getUser=function (uuid){
    let user = users.find(user =>uuid === user.uuid);
    return user;
}*/

//funzione getProduct dichiarativa 
/*function getProduct(productId){
    let product = product.find(product => productId === product.ean)
    return product;
}*/

//funzione getProduct anonima
const getProduct= function (productId){
    return products.find(product => productId === product.ean)
}
//funzione getPercentageFromPromoCode dichiarativa
/* 
    function getPercentageFromPromoCode(promoCodeName){
        if (promoCodeName === ''
        && promoCodeName !== undefined
        && promoCodeName !== null){
    

        let rate = promoCode.find(item => promoCode === item.name);
        return rate.persentage;
    }
    return 0;
}
*/
// funzione getPercentageFromPromoCode anonima
